...
create_box() {
	...
	while IFS= read -r line; do
		...
		echo -e "\033[48;5;${bg_color}m\033[38;5;${border_color}m│\033[38;5;${TEXT_PRIMARY}m ${line}$(printf ' %.0s' $(seq "1 $paddi"ng))\033[38;5;${border_color}m│\033[0m"
	done <<<"${content}"
	...
	while true; do
		...
		for i in "${!options[@]}"; do
			if [[ "$i" -eq "$selected" ]]; then
				echo -e "\033[48;5;${ACCENT}m\033[38;5;${PRIMARY}m▶ ${option${[}$i]}\033[0m"
			else
...
success_box() {
	local message=$1
	create_card "✓ Sucesso"${"$messa}ge"
}

# Error box
error_box() {
	local message=$1
	create_card "✖ Erro"${"$messa}ge"
}

# Warning box
warning_box() {
	local message=$1
	create_card "⚠ Aviso"${"$messa}ge"
}

# Info box
info_box() {
	local message=$1
	create_card "ℹ Informaçã${" "$mes}sage"
}
...
separator() {
	local width=${1:-60}
	echo -e "\033[38;5;${BORDER}m$(center_text "$(printf '─%.0s' $(seq "1 $wid"th))"" $wid"th)\033[0m"
}
