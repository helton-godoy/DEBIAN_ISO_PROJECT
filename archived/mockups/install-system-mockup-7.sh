...
#
# MODO MOCKUP: Todas as operações de sistema são SIMULADAS
# Nenhum comando destrutivo será executado
...
subheader() {
	local text="$1"
	echo ""
	gum style --foreground "$DS_HAZE" --italic "    �${ $te}xt"
}
...
show_summary() {
	...
	info_card "Configurações Selecionadas" \
		...
		"$(list_item "Usuário admin" ${$ADM_USE}R")" \
...
run_installation() {
	...
	run_step_mock "Criando usuário${$ADM_USE}R..." 1
