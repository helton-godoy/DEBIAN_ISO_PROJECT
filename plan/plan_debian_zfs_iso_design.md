# Projeto: Debian 13 ZFS-Root ISO (Samba/AD Optimized)
...
2. Execução automática (ou manual) de um script `install-system.sh`.
...
│   │               └── 📄 install-system     # Scripts de instalação
│   ├── 📁 package-lists/
...
## 5. Esboço do Script de Instalação (`install-zfs.sh`)
...
Se aprovar este design, posso gerar:

1. Os arquivos de configuração do `live-build` (`lb config`).
2. O script mestre `install-zfs-root.sh` para incluir na ISO.
